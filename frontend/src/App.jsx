...
function App() {
  ...
  // Advanced settings
  const [ttaRuns, setTtaRuns] = useState(1);
  const [mcRuns, setMcRuns] = useState(5);
  ...
  async function onSubmit(event) {
    ...
    if (!file) {
      setError("Upload or capture a lesion image first.");
      return;
    }
    setError("");
    setIsLoading(true);
    try {
      ...
      const response = await axios.post(
        `${API_BASE}/predict?explain=true&tta_runs=${ttaRuns}&mc_runs=${mcRuns}&include_probabilities=true&include_bias_analysis=true`,
        formData,
  ...
  return (
    <main className="page">
      <section className="hero">
        ...
        <h1>UNIFIED SKIN LESION INTELLIGENCE</h1>
        <p className="hero-subtitle">
          Advanced AI-Powered Dermatology Analysis with Bias Awareness
        </p>
        <p>
          Upload or capture a lesion image, run inference with Monte Carlo Dropout uncertainty
          quantification, Grad-CAM explainability, and Fitzpatrick skin tone bias analysis.
        </p>
      </section>
      ...
      <section className="panel">
        <form onSubmit={onSubmit} className="upload-form">
          ...
          {cameraActive && (
            <div className="camera-wrap">
              ...
              <button type="button" className="capture-btn" onClick={captureFrame}>
                Capture Frame
              </button>
            </div>
          )}
          ...
          {showAdvanced && (
            <div className="advanced-settings">
              <h3>Advanced Inference Settings</h3>
              <div className="setting-row">
                <label htmlFor="tta">
                  Test-Time Augmentation (TTA) Runs: {ttaRuns}
                  <input
                    id="tta"
                    type="range"
                    min="1"
                    max="8"
                    value={ttaRuns}
                    onChange={(e) => setTtaRuns(parseInt(e.target.value))}
                  />
                </label>
              </div>
              <div className="setting-row">
                <label htmlFor="mc">
                  Monte Carlo Dropout Runs: {mcRuns}
                  <input
                    id="mc"
                    type="range"
                    min="1"
                    max="10"
                    value={mcRuns}
                    onChange={(e) => setMcRuns(parseInt(e.target.value))}
                  />
                </label>
              </div>
              <p className="setting-note">
                Higher values improve prediction stability but increase inference time.
              </p>
            </div>
          )}
        ...
        <div className="grid">
                  ...
                  <div className={`uncertainty-box ${uncertaintyColor}`}>
                    ...
                    <div className="uncertainty-bar-track">
                      <div
                        ...
                        style={{ width: `${result.uncertainty.score * 100}%` }}
                      />
                    </div>
                    <p className="uncertainty-meta">
                      {(result.uncertainty.score * 100).toFixed(1)}% —{" "}
                      {result.uncertainty.level} uncertainty
                    </p>
                    {result.uncertainty.level === "high" && (
                      <p className="uncertainty-warning">
                        ⚠️ High uncertainty — specialist review recommended.
                      </p>
                    )}
                  </div>
                )}
                      ...
                      <li key={item.label}>
                        <div className="prob-head">
                          ...
                          <span>
                            ...
                            {item.std != null ? ` ±${(item.std * 100).toFixed(1)}` : ""}
                          </span>
                        </div>
          ...
          {result?.fitzpatrick_analysis && (
            <article className="card card-wide bias-analysis">
              <h2>🌍 Fitzpatrick Skin Tone Bias Analysis</h2>
              <div className="bias-grid">
                <div className="bias-metric">
                  <span className="bias-label">Detected Skin Tone:</span>
                  <span className="bias-value">
                    {result.fitzpatrick_analysis.skin_tone_name}
                  </span>
                </div>
                <div className="bias-metric">
                  <span className="bias-label">Training Data Representation:</span>
                  <span className="bias-value">
                    {result.fitzpatrick_analysis.training_representation}
                  </span>
                </div>
                <div className="bias-metric">
                  <span className="bias-label">Model Reliability:</span>
                  <span
                    className={`reliability-badge ${result.fitzpatrick_analysis.reliability_level.toLowerCase()}`}
                  >
                    {result.fitzpatrick_analysis.reliability_level}
                  </span>
                </div>
                <div className="bias-metric">
                  <span className="bias-label">Original Confidence:</span>
                  <span className="bias-value">
                    {result.fitzpatrick_analysis.original_confidence}
                  </span>
                </div>
                <div className="bias-metric">
                  <span className="bias-label">Bias-Adjusted Confidence:</span>
                  <span className="bias-value adjusted">
                    {result.fitzpatrick_analysis.adjusted_confidence}
                  </span>
                </div>
              </div>
              {result.fitzpatrick_analysis.bias_warning && (
                <div
                  className={`bias-warning ${
                    result.fitzpatrick_analysis.reliability_level === "LOW" ? "critical" : ""
                  }`}
                >
                  {result.fitzpatrick_analysis.bias_warning}
                </div>
              ...
              <div className="bias-recommendation">
                <strong>Recommendation:</strong> {result.fitzpatrick_analysis.recommendation}
              </div>
              <div className="bias-note">
                <strong>Dataset Bias Note:</strong>{" "}
                {result.fitzpatrick_analysis.dataset_bias_note}
              </div>
            </article>
          )}
        </div>
        ...
        <div className="notice">
          <strong>Clinical Safety Notice:</strong> This tool is for research and educational
          purposes only. It is not a medical device. Final diagnosis must always be performed by a
