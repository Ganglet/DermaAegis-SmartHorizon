...
async def predict(
    ...
    tta_runs: int = Query(default=1, ge=1, le=4),
    ...
    if explain:
        try:
            ...
            gradcam_payload["gradcam_base64"] = _overlay_gradcam(input_f32[0], heatmap)
        except Exception as exc:
            gradcam_payload["gradcam_error"] = str(exc)

    # 4. MC Dropout only for per-class std (uncertainty display)
