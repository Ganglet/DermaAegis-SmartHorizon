...
BACKBONES = {
    ...
    "efficientnetb3": BackboneSpec(
        name="EfficientNetB3",
        model_cls=tf.keras.applications.EfficientNetB3,
        preprocess_input=tf.keras.applications.efficientnet.preprocess_input,
    ),
...
def make_gradcam_heatmap(
    ...
    """Generate a saliency heatmap via input gradients.

    Input-gradient method is robust across all backbone types (including nested
    EfficientNet functional models) and works with mixed_float16 policies.
    The last_conv_layer_name parameter is kept for API compatibility but unused.
    """
    img_tensor = tf.Variable(tf.cast(img_array, tf.float32), trainable=True)
    ...
    if input_grads is None:
        raise ValueError("Input gradients are None — model may not support differentiation")

    heatmap = tf.reduce_mean(tf.math.abs(input_grads[0]), axis=-1)
